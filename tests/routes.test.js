	...
	build().then(() => {
		supertest(server)
			...
			.end((err, res) => {
				t.error(
					err,
					"Status is 200 and content-type is text/html; charset=utf-8"
				);
				t.equals(
					res.text.includes("<h1>Link Hoarder</h1>"),
					true,
					"<h1>HOME</h1> should be present"
				);
				t.end();
			});
	});
	...
	build().then(() => {
		supertest(server)
			...
			.end((err, res) => {
				...
				t.equals(
					res.text.includes('class="form signup-form"'),
					true,
	...
	build().then(() => {
		supertest(server)
			...
			.end((err, res) => {
				...
				t.equals(
					res.text.includes('class="form login-form"'),
					true,
